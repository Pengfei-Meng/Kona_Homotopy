...
class ReducedKKTVector(CompositeVector):
    ...
    def equals_KKT_conditions_reduced(self, x, state, adjoint, CompositeReduced, ineq_work, ineq_work2,
                             design_work, barrier=None,  obj_scale=1.0, cnstr_scale=1.0):

        assert isinstance(x.primal, CompositePrimalVector), \
            "ReducedKKTVector()_reduced >> KKT point Reduced must include slacks!"
        at_design = x.primal.design
        at_slack = x.primal.slack

        assert isinstance(x.dual, CompositeDualVector), \
            "ReducedKKTVector()_reduced >> KKT point cannot include inequality!"        
        at_dual_eq = x.dual.eq
        at_dual_ineq = x.dual.ineq

        assert isinstance(CompositeReduced.primal, DesignVector), \
            "ReducedKKTVector()_reduced >> CompositeReduced.primal must be of DesignVector type!"

        assert isinstance(CompositeReduced.dual, DualVectorEQ), \
            "ReducedKKTVector()_reduced >> CompositeReduced.dual must be of DualVectorEQ type!"

        assert isinstance(ineq_work, DualVectorINEQ), \
            "ReducedKKTVector()_reduced >> ineq_work must be of DualVectorINEQ type!"
            

        CompositeReduced.primal.equals_lagrangian_total_gradient(
            x.primal, state, x.dual, adjoint, obj_scale, cnstr_scale)

        ineq_work.equals_constraints(x.primal, state)  

        ineq_work2.equals(at_slack)
        ineq_work2.pow(-1.)
        ineq_work2.times(at_dual_ineq)
        
        ineq_work.times(ineq_work2)

        dCINdX_mat = dCINdX()
        dCINdX_mat.linearize(x.primal, state)
        dCINdX_mat.T.product(ineq_work, design_work)

        CompositeReduced.primal.minus(design_work)
        CompositeReduced.dual.equals_constraints(x.primal, state)
...
from kona.linalg.matrices.common import dCINdX
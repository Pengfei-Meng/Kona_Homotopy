...
class dRdU(KonaMatrix):
    ...
    def solve(self, rhs_vec, solution, rel_tol=1e-8):
        ...
        if not self._transposed:
            ...
            # print 'cost for solve linear', cost
        else:
            cost = self._solver.solve_adjoint(
                self._design.base.data, self._state.base,
                rhs_vec.base, rel_tol, solution.base)
            # print 'cost for solve adjoint', cost
        if cost >= 0:
            converged = True
...
## expedient way of taking explicit linear constraint Jacobian, nonlinear products
class dCdX_total_linear(KonaMatrix):
    """
    accessing the total gradient for linear constraints from DVCon
    """
    def explicit_mat(self):
        assert self._linearized
        g_eq, g_ineq = self._solver.total_dCdX_linear(
            self._design.base.data, self._state.base)
        return g_eq, g_ineq

class dCINdX_nonlinear(KonaMatrix):
    def product(self, in_vec, out_vec):
        assert self._linearized
        if not self._transposed:
            if isinstance(in_vec, CompositePrimalVector):
            ...
            assert isinstance(out_vec, np.ndarray), \
                "Invalid output vector: must be np.ndarray!"
            out_vec = self._solver.multiply_dCINdX_nonlinear(
                self._design.base.data, self._state.base,
                in_design.base.data)

        else:
            assert isinstance(in_vec, np.ndarray), \
                "Invalid multiplying vector: must be np.ndarray!"
            if isinstance(out_vec, CompositePrimalVector):
                out_design = out_vec.design
            elif isinstance(out_vec, DesignVector):
                out_design = out_vec
            ...
            out_design.base.data[:] = self._solver.multiply_dCINdX_T_nonlinear(
                self._design.base.data, self._state.base,
                in_vec)

class dCINdU_nonlinear(KonaMatrix):
    def product(self, in_vec, out_vec):
        assert self._linearized
        if not self._transposed:
            assert isinstance(in_vec, StateVector), \
            ...
            assert isinstance(out_vec, np.ndarray), \
                "Invalid output vector: must be np.ndarray!"
            out_vec = self._solver.multiply_dCINdU_nonlinear(
                self._design.base.data, self._state.base,
                in_vec.base)
        else:
            assert isinstance(in_vec, np.ndarray), \
            ...
            assert isinstance(out_vec, StateVector), \
                "Invalid output vector: must be StateVector!"
            self._solver.multiply_dCINdU_T_nonlinear(
                self._design.base.data, self._state.base,
                in_vec, out_vec.base)    

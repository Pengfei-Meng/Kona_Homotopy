...
for i in xrange(num_design):
    ...
    W_full[:, i] = out_design._data.data.x[:, 0]
...
# -------- saving the constraint values, the dual component of kkt_rhs EQU
cnstr_val = df.generate()
cnstr_val.equals_constraints(at_design, at_state)
cnstr_val.times(-1.0)

con_vals = np.zeros((num_dual, 1))
con_vals[:num_design,0]             = cnstr_val._data.x_lower.x[:, 0]
con_vals[num_design:2*num_design,0] = cnstr_val._data.x_upper.x[:, 0]
con_vals[2*num_design:, 0]           = cnstr_val._data.stress.x[:, 0]


con_file = open('rhs_cnstr_noS', 'w')
pickle.dump(con_vals, con_file)
con_file.close()

# --------- saving lagrangian_total_gradient, the primal component of kkt_rhs EQU
dldx = pf.generate()
primal_work = pf.generate()

dldx.equals_lagrangian_total_gradient(at_design, at_state, at_dual,
                                         at_adjoint, primal_work)
dldx.times(-1.0)

dldx_vals = np.zeros((num_design, 1))
dldx_vals[:,0] = dldx._data.data.x[:,0]

dldx_file = open('rhs_dldx_noS', 'w')
pickle.dump(dldx_vals, dldx_file)
dldx_file.close()
